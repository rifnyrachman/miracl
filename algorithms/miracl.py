...
class MIRACLConfig(AlgorithmConfig):
    ...
    def __init__(self, algo_class=None):
        ...
        super().__init__(algo_class=algo_class or MIRACL)
...
# @mluo: TODO
#modification: add weight initialisation and DM-based weight updates
def set_worker_tasks(workers, use_meta_env):
    if use_meta_env:
        ...
        weights = {}  # Store weights for each task

        # Initialize weights for tasks
        for i in range(n_tasks):
            weights[i] = np.random.uniform(size=(len(tasks[i].vector_reward)))  # Random initial weights
            weights[i] /= weights[i].sum()  # Normalize weights
            #tasks[i].weights = weights[i]

        for i, worker in enumerate(workers.remote_workers()):
            task_id = i
            worker.foreach_env.remote(lambda env, i=i : env.set_task(tasks[i], weight[i], task_id))
            worker.foreach_env.remote(lambda env: env.set_weights(weights[i]))

class MetaUpdate:
    def __init__(self, workers, maml_steps, metric_gen, use_meta_env, delta=1.05):
        self.workers = workers
        self.maml_optimizer_steps = maml_steps
        self.metric_gen = metric_gen
        self.use_meta_env = use_meta_env
        self.delta = delta  # DM adjustment factor
        self.task_history = {}  # To track non-dominated archives for tasks
        self.current_iteration = 0  # Track iterations #additional
        self.generated_tasks = [] #store tasks generated by SimpleSC
        self.sampled_task_weights = {} #store weights for current sampled task
        
    def generate_initial_tasks(self, env, num_tasks=99):
        """Generate 99 tasks using SimpleSC's task generator."""
        self.generated_tasks = env.sample_tasks(num_tasks)
        print(f"Generated {len(self.generated_tasks)} tasks.")

    def sample_task(self):
        """Sample a single task from the 99 tasks."""
        if len(self.generated_tasks)==0:
            raise ValueError("No tasks available. Generate tasks first.")
        return random.choice(self.generated_tasks)

    def initialize_weights_for_task(self, task, num_weights=10):
        """Generate 10 random weights for the sampled task."""
        task_id = id(task)
        weights = []
        for _ in range(num_weights):
            w = np.random.uniform(size=(3,))  # Assume 3 objectives
            w /= np.sum(w)  # Normalize weights
            weights.append(w)
        self.sampled_task_weights[task_id] = weights
        print(f"Initialized {len(weights)} weights for task {task_id}.")
        return task_id
    
    def adjust_weights_dm(self, task_id, vector_rewards, steps=5):
        """Perform DM updates on the 10 weights for the task."""
        if task_id not in self.sampled_task_weights:
            raise ValueError(f"Task ID {task_id} not found in sampled weights.")

        # Ensure the task has an archive in task_history
        if task_id not in self.task_history:
            self.task_history[task_id] = []

        weights = self.sampled_task_weights[task_id]
        for step in range(steps):
            for i, weight in enumerate(weights):
                # Archive for this task
                archive = np.array(self.task_history[task_id]) if self.task_history[task_id] else np.empty((0, len(weight)))
                vector_reward = vector_rewards[i]

                if len(archive) > 0:
                    # Find closest non-dominated neighbor
                    distances = np.linalg.norm(archive - vector_reward, axis=1)
                    closest_idx = np.argmin(distances)
                    closest_non_dominated = archive[closest_idx]

                    # Perform DM adjustment
                    for j in range(len(weight)):
                        if vector_reward[j] >= closest_non_dominated[j]:
                            weight[j] *= self.delta
                        else:
                            weight[j] /= self.delta

                    # Normalize the weight
                    weights[i] = weight / max(np.sum(weight), 1e-8)

                # Add the vector_reward to the archive if it's non-dominated
                is_non_dominated = all(
                    not np.all(vector_reward <= other) or np.any(vector_reward < other)
                    for other in archive
                )
                if is_non_dominated:
                    self.task_history[task_id].append(vector_reward)

                print(f"Step {step + 1}: Updated weight {i} for task {task_id}: {weights[i]}")


    def set_weights_on_workers(self, task_id):
        """Set weights on workers without using lambdas."""
        def update_weights(env, task_id=task_id, weights=self.sampled_task_weights):
            env.set_weights({task_id: weights[task_id]})

        for worker in self.workers.remote_workers():
            worker.foreach_env.remote(update_weights)
    ...
    def __call__(self, data_tuple):
        ...
        # Perform MAML optimization steps
        for i in range(self.maml_optimizer_steps):
            fetches = self.workers.local_worker().learn_on_batch(samples)
        learner_stats = get_learner_stats(fetches)

        # Sync workers with the updated meta-policy
        self.workers.sync_weights()

        # Generate tasks if not already generated
        env = self.workers.local_worker().foreach_env(lambda e: e)[0]
        if len(self.generated_tasks)==0:
            self.generate_initial_tasks(env)

        # Sample a task and initialize weights
        sampled_task = self.sample_task()
        task_id = self.initialize_weights_for_task(sampled_task)

        # Simulate vector_rewards for 10 weights (placeholder logic)
        vector_rewards = [
            np.random.uniform(size=(3,)) for _ in range(len(self.sampled_task_weights[task_id]))
        ]

        # Perform DM adjustments
        self.adjust_weights_dm(task_id, vector_rewards)

        # Update worker environments with the adjusted weights
        self.set_weights_on_workers(task_id)
        
        # Debugging outputs
        print(f"Task {task_id} weights after DM: {self.sampled_task_weights[task_id]}")
        print(f"Task {task_id} archive: {self.task_history[task_id]}")
        
        # Update KL coefficients for policies
        def update(pi, pi_id):
        ...
        # Initialize metrics keys if they do not exist
        metrics.counters[STEPS_TRAINED_COUNTER] = (
            metrics.counters.get(STEPS_TRAINED_COUNTER, 0) + samples.count
        )
        metrics.counters[STEPS_TRAINED_THIS_ITER_COUNTER] = (
            metrics.counters.get(STEPS_TRAINED_THIS_ITER_COUNTER, 0) + samples.count
        )
        metrics.counters[STEPS_SAMPLED_COUNTER] = (
            metrics.counters.get(STEPS_SAMPLED_COUNTER, 0) + samples.count
        )



        # Combine results with the adaptation metrics
...
class MIRACL(Algorithm):
    ...
    def get_default_policy_class(
        ...
        if config["framework"] == "torch":
            from algo_ray.rllib.algorithms.maml.maml_torch_policy import MAMLTorchPolicy #use NaN-remover torch policy
...
class _deprecated_default_config(dict):
    def __init__(self):
        super().__init__(MIRACLConfig().to_dict())
