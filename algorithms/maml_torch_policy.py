...
from algo_ray.rllib.models.torch.torch_action_dist_loc import TorchDistributionWrapperLoc
from ray.rllib.policy.sample_batch import SampleBatch
from ray.rllib.policy.torch_mixins import ValueNetworkMixin
from algo_ray.rllib.policy.torch_policy_v2 import TorchPolicyV2
...
def PPOLoss(
    ...
    def safe_tensor(tensor, default=0.0): #additional function to remove NaN values
        if torch.isnan(tensor).any() or torch.isinf(tensor).any():
            print(f"Invalid tensor detected: {tensor}")
            return torch.full_like(tensor, default)
    ...
    def surrogate_loss(
        ...
        pi_new_logp = safe_tensor(curr_dist.logp(actions))
        pi_old_logp = safe_tensor(prev_dist.logp(actions))
    ...
    def kl_loss(curr_dist, prev_dist):
        return safe_tensor(prev_dist.kl(curr_dist))

    def entropy_loss(dist):
        return safe_tensor(dist.entropy())

    def vf_loss(value_fn, value_targets, vf_preds, vf_clip_param=0.1):
        # GAE Value Function Loss
        vf_loss1 = safe_tensor(torch.pow(value_fn - value_targets, 2.0))
        vf_clipped = vf_preds + torch.clamp(
            value_fn - vf_preds, -vf_clip_param, vf_clip_param
        )
        vf_loss2 = safe_tensor(torch.pow(vf_clipped - value_targets, 2.0))
        vf_loss = torch.max(vf_loss1, vf_loss2)
        return vf_loss
    
    curr_logits = safe_tensor(curr_logits)
    behaviour_logits = safe_tensor(behaviour_logits)
...
class MAMLTorchPolicy(ValueNetworkMixin, KLCoeffMixin, TorchPolicyV2):
    ...
    def loss(
        self,
        model: ModelV2,
        dist_class: Type[TorchDistributionWrapperLoc],
        ...
        # Detect NaN in logits
        if torch.isnan(logits).any():
            print("NaN detected in logits. Resetting model parameters.")
